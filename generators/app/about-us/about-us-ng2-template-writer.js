...
function write(generator) {
    ...
    generator.variableName = _.upperFirst(_.camelCase(jhipsterAppConfig.variableName));

    const cName = generator.userInputClassName;
    generator.userInputClassName = _.upperFirst(_.camelCase(cName));
    ...
    generator.log(`User gave this input=${s}`);
    generator.log(`User gave desctription=${generator.decriptionInput}`);
    generator.log(`User gave component directory name=${componentDirName}`);
    generator.log(`User gave component directory name=${generator.componentDirName}`);
    generator.log(`User gave Class Name=${generator.userInputClassName}`);
    generator.log(`User gave Variable Name=${generator.userVarName}`);
    generator.log('------------------------------------------------------------');

    // HTML TEMPLATE
    if (generator.wantWebApp) {
        generator.template(
        ...
        // MY FILE
        generator.template(
            `${ng2TemplateDir}src/main/webapp/app/element/element.componentcopy.ts`,
            `${webappDir}app/${componentDirName}/${componentName}.componentcopy.ts`
        );
    ...
    // JAVA CLASS
    generator.template(
        `${ng2TemplateDir}src/main/java/app/xiges/XigesSampleClass.java`,
        `${javaDir}app/xiges/sample/${componentDirName}/${cName}.java`
    );
